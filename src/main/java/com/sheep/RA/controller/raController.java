package com.sheep.RA.controller;



import com.sheep.RA.crypto.AES;
...
//    public Map<String, String> disclosure(@PathVariable String params)
    {
        ...
        String ts = split_disclosure_message[2];
        String ecdsaSig = split_disclosure_message[3];

        if((validateSignature(disclosureSeed + "-" + disclosureGid + "-" + ts, publicKeyOfTa , Hex.decode(ecdsaSig))) && checkTimeStamp(ts))
        {
            System.out.println("validate success");
        //3.從資料庫中找到此seed相對應的vid
            String disclosureVid = getCorrespondVid(disclosureGid, disclosureSeed);
        //4.generate correspond sig         vid-ts-sig
            System.out.println("disclosure vid" + disclosureVid);
            ts = GetTimeStamp();
            String tempPlainText = disclosureVid + "-" + ts;
            String respondSig = Hex.toHexString(generateEcdsaSignature(tempPlainText, privateKeyOfRa));
            byte[] respondindCipher = aes.encrypt(concatenateByteArray((tempPlainText + "-").getBytes(), respondSig.getBytes()));
...
//        System.out.println(System.currentTimeMillis() + "                      first stamp");
//        System.out.println(Long.toString(System.currentTimeMillis()) + "                      first stamp");
        return Long.toString(System.currentTimeMillis());
    }

    public static boolean checkTimeStamp(String oldStamp)
    {
//        Timestamp newStamp = new Timestamp((new Date().getTime()));
        long newStamp = System.currentTimeMillis();
        Timestamp oldStamp2 = new Timestamp(Long.parseLong(oldStamp));
        Timestamp newStamp2 = new Timestamp(newStamp);
        System.out.println(newStamp2.getTime() -  oldStamp2.getTime() + "兩stamp相減");
        if((newStamp2.getTime() -  oldStamp2.getTime()) < 4000)
            return true;
        else return false;
    }
